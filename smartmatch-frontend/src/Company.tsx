          ...
          <motion.div key="menu4" variants={containerVariants} initial="hidden" animate="show" exit="hidden" className="w-full max-w-6xl mx-auto">
            ...
            <div className="flex flex-col gap-8 lg:flex-row">
              <motion.div variants={itemVariants} className="flex-1 p-8 bg-white/70 dark:bg-[#121212] backdrop-blur-xl border shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-none rounded-[2rem] border-zinc-200/50 dark:border-white/5">
                ...
                <textarea value={jd} onChange={e=>setJd(e.target.value)} className="w-full h-40 p-5 mb-6 text-sm transition-all bg-white dark:bg-[#0a0a0a] border shadow-sm dark:shadow-none outline-none resize-none text-zinc-700 dark:text-zinc-200 border-zinc-200/80 dark:border-white/10 rounded-2xl focus:ring-2 focus:ring-indigo-500/20" />
                ...
                <div className="pt-8 border-t border-zinc-100 dark:border-white/5">
                  <form onSubmit={handlePostJob}>
                    <input type="text" placeholder="ตำแหน่งงานจริง เช่น Software Engineer" value={newJob.title} onChange={e=>setNewJob({...newJob, title:e.target.value})} className="w-full p-4 mb-4 text-sm transition-all bg-zinc-50 dark:bg-[#0a0a0a] border border-zinc-200/80 dark:border-white/10 outline-none rounded-2xl focus:ring-2 focus:ring-indigo-500/20 text-zinc-800 dark:text-zinc-200" required />
                    <input type="text" placeholder="ชื่อบริษัท" value={profileData.companyName} readOnly className="w-full p-4 mb-8 text-sm transition-all bg-zinc-100 dark:bg-[#1a1a1a] border border-zinc-200/80 dark:border-white/5 outline-none rounded-2xl text-zinc-500 dark:text-zinc-500 cursor-not-allowed" />
